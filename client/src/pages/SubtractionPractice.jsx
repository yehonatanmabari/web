...
import {
  ...
  LEVEL_TEXT,
  makeQuestion,
  levelFromSubtractionF,
  fetchSubtractionF,
  fetchIncSubtraction
} from "./subtractionLogic";
import { savePracticeState, clearPracticeState, getPracticeState } from "./practiceState";

export default function SubtractionExampleBetter() {
  ...
  function saveState(next = {}) {
    savePracticeState(SUB_STATE_KEY, { level, q, input, msg, noPointsThisQuestion, story }, next);
  }
  function clearState() {
    clearPracticeState(SUB_STATE_KEY);
  }

  useEffect(() => {
    const st = getPracticeState(SUB_STATE_KEY);
    if (st) {
  ...
  function goStory() {
    ...
    const s =
      `מתי החתול אומר 😺:\n` +
      `בתרגיל הזה יש לנו ${q.a} פחות ${q.b}.\n` +
      `אפשר לחשוב: אם יש לי ${q.a} עוגיות,\n` +
      `ואני נותן ${q.b} לחברים — נשאר ${q.ans}.\n` +
